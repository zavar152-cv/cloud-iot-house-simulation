...
export const CurtainsSettings = ({actions}) => {
	...
	const [group, setGroup] = useState("CURTAINS_GROUP")
	let getTimetable = () => {
	...
	let saveTimeSlot = (timeslot) => {
		...
		let id = uuidv4();
		saveCommand(id + '_on',
				actions.filter(action => action.name === 'curtains_up')[0].id,
				cronExpressions[0]);
		saveCommand(id + '_off',
				actions.filter(action => action.name === 'curtains_down')[0].id,
				cronExpressions[1]);
		0;
	};
		...
		<Paper elevation={1}>
			...
			>
				<Grid
					...
					width='94%'
				>
					<Typography variant='h6'>Расписание работы штор:</Typography>
				</Grid>
				{timetable.map((e) => {
					return (
						<Grid
					...
					<Fab
						...
						onClick={() => {
							setTimetable([
								...timetable,
								{
									id: timetable.length === 0 ? 0 : timetable[timetable.length - 1].id + 1,
									dayOfWeek: 0,
									from: '00:00',
									to: '00:01',
									isNew: true,
